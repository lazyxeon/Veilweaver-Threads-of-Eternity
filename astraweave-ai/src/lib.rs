            ...
            if smoke_cd <= 0.0 {
                return PlanIntent {
                    ...
                    steps: vec![
                        ...
                        ActionStep::MoveTo { x: m.pos.x + (first.pos.x - m.pos.x).signum()*2,
                                             y: m.pos.y + (first.pos.y - m.pos.y).signum()*2 },
                        ActionStep::CoverFire { target_id: first.id, duration: 2.5 },
                    ],
                };
            } else {
                // advance cautiously
                return PlanIntent {
                    plan_id,
                    steps: vec![
                        ActionStep::MoveTo { x: m.pos.x + (first.pos.x - m.pos.x).signum()*1,
                                             y: m.pos.y + (first.pos.y - m.pos.y).signum()*1 },
                        ActionStep::CoverFire { target_id: first.id, duration: 1.5 },
                    ],
                };
            }
        }
