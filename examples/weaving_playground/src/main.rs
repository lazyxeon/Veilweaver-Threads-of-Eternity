...
fn main() -> anyhow::Result<()> {
    ...
    let mut camera = Camera {
        position: vec3(-4.0, 7.0, 12.0), yaw: -3.14/2.1, pitch: -0.55,
        fovy: 60f32.to_radians(), aspect: 16.0/9.0, znear:0.1, zfar: 400.0
    };
    ...
    // Weave budget
    let mut budget = WeaveBudget { terrain_edits: 3, weather_ops: 2 };
            ...
            Event::WindowEvent { event, .. } => match event {
                ...
                WindowEvent::KeyboardInput { event: KeyEvent{ state, physical_key: PhysicalKey::Code(code), .. }, .. } => {
                    ...
                    if down {
                        ...
                        match code {
                            KeyCode::Digit1 => {
                                let op = WeaveOp { kind: WeaveOpKind::ReinforcePath, a: vec3(2.0,0.0,2.0), b: None, budget_cost:1 };
                                if let Ok(cons) = apply_weave_op(&mut w, &mut phys, &tris, &mut budget, &op, &mut log) {
                                    println!("Consequence: drop x{}, faction {}", cons.drop_multiplier, cons.faction_disposition);
                                }
                            }
                            KeyCode::Digit2 => {
                                let op = WeaveOp { kind: WeaveOpKind::CollapseBridge, a: vec3(1.0,0.0,-1.0), b: Some(vec3(6.0,0.0,-1.0)), budget_cost:1 };
                                let _ = apply_weave_op(&mut w, &mut phys, &tris, &mut budget, &op, &mut log);
                            }
                            KeyCode::Digit3 => {
                                let op = WeaveOp { kind: WeaveOpKind::RedirectWind, a: vec3(0.0,0.0,0.0), b: Some(vec3(1.0,0.0,0.2)), budget_cost:1 };
                                let _ = apply_weave_op(&mut w, &mut phys, &tris, &mut budget, &op, &mut log);
                            }
                            KeyCode::Digit4 => {
                                let op = WeaveOp { kind: WeaveOpKind::LowerWater, a: vec3(0.0,0.0,0.0), b: None, budget_cost:1 };
                                let _ = apply_weave_op(&mut w, &mut phys, &tris, &mut budget, &op, &mut log);
                            }
                            _ => {}
                        }
            ...
            Event::AboutToWait => {
                ...
                for (x,y) in w.obstacles.iter() {
                    instances.push(Instance::from_pos_scale_color(
                        glam::vec3(*x as f32, 0.5, *y as f32), glam::vec3(0.9,1.0,0.9),
                        [0.45,0.45,0.45,1.0]
                    ));
                }
                for e in w.all_of_team(0) {
                    let p = w.pos_of(e).unwrap();
                    instances.push(Instance::from_pos_scale_color(glam::vec3(p.x as f32,0.5,p.y as f32), glam::vec3(0.7,1.0,0.7), [0.2,0.4,1.0,1.0]));
                }
                for e in w.all_of_team(1) {
                    let p = w.pos_of(e).unwrap();
                    instances.push(Instance::from_pos_scale_color(glam::vec3(p.x as f32,0.5,p.y as f32), glam::vec3(0.7,1.0,0.7), [0.2,1.0,0.4,1.0]));
                }
                for e in w.all_of_team(2) {
                    let p = w.pos_of(e).unwrap();
                    instances.push(Instance::from_pos_scale_color(glam::vec3(p.x as f32,0.5,p.y as f32), glam::vec3(0.7,1.0,0.7), [1.0,0.2,0.2,1.0]));
                }
