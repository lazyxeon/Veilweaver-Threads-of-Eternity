...
async fn run() -> Result<()> {
    ...
    let mut elwt = event_loop;
    elwt.run(move |event, _, control_flow| {
                    ...
                    WindowEvent::KeyboardInput { event: KeyEvent { physical_key, state, .. }, .. } => {
                        ...
                        match physical_key {
                            ...
                            PhysicalKey::Code(KeyCode::Escape) => {
                                if pressed { *control_flow = winit::event_loop::ControlFlow::Exit; }
                            }
                            ...
                            PhysicalKey::Code(KeyCode::KeyE) => {
                                if pressed {
                                    ...
                                    let origin = r3::Point::new(camera.pos.x, camera.pos.y, camera.pos.z);
                                    let dir = r3::Vector::new(forward.x, forward.y, forward.z);
                                    physics.query_pipeline.update(&physics.islands, &physics.bodies, &physics.colliders);
                                    if let Some((h, _toi)) = physics.query_pipeline.cast_ray(
                                        ...
                                        if let Some(body) = physics.bodies.get_mut(h.0) {
                                            if !body.is_fixed() {
                                                let impulse = r3::Vector::new(forward.x * 3.0, 1.0, forward.z * 3.0);
...
fn fs_main(in: VsOut) -> @location(0) vec4<f32> {
  ...
  if (dist < 0.51) {
    let scale = 1.5;
    let cx = floor(in.world_pos.x / scale);
    let cz = floor(in.world_pos.z / scale);
    let checker = i32(cx + cz) & 1;
    let c1 = vec3<f32>(0.22, 0.22, 0.22);
    let c2 = vec3<f32>(0.28, 0.28, 0.28);
    col = mix(c1, c2, f32(checker));
  }
  // Sky ambient
