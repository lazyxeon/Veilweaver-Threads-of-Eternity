...
use astraweave_npc::{
    NpcManager, EngineCommandSink, NpcWorldView,
        ...
        match event {
            Event::WindowEvent { event, .. } => match event {
                ...
                WindowEvent::KeyboardInput { event: KeyEvent{ state, physical_key: PhysicalKey::Code(code), .. }, .. } => {
                    ...
                    if down {
                        match code {
                            KeyCode::KeyH => { utter_hello = true; utter_buy = false; utter_danger = false; println!("Utterance: hello"); }
                            KeyCode::KeyB => { utter_hello = false; utter_buy = true; utter_danger = false; println!("Utterance: buy/shop"); }
                            KeyCode::KeyD => { utter_hello = false; utter_buy = false; utter_danger = true; println!("Utterance: danger/help"); }
                            KeyCode::KeyE => {
                                // talk to nearest (merchant or guard). For demo, alternate:
                                let mut glue = EngineCommandSink { phys: &mut phys, audio: &mut audio };
                                ...
                                let utter = if utter_hello { "hello" } else if utter_buy { "buy" } else { "danger" };
                                let _ = npcs.handle_player_utterance(merchant_id, &view_merchant, utter);
            ...
            Event::AboutToWait => {
                ...
                let views = std::iter::once((merchant_id, NpcWorldView {
                        time_of_day: 12.0, self_pos: vec3(0.0,1.0,0.0),
                        player_pos: Some(camera.position), player_dist: Some(camera.position.distance(vec3(0.0,1.0,0.0))),
                        nearby_threat: utter_danger, location_tag: Some("market".into())
                    }))
                    .chain(std::iter::once((guard_id, NpcWorldView {
                        time_of_day: 12.0, self_pos: vec3(3.0,1.0,0.0),
                        player_pos: Some(camera.position), player_dist: Some(camera.position.distance(vec3(3.0,1.0,0.0))),
                        nearby_threat: utter_danger, location_tag: Some("gate".into())
                    })))
                    .collect();
                ...
                instances.push(Instance::from_pos_scale_color(vec3(0.0,0.5,0.0), vec3(0.6,1.0,0.6), [0.2,1.0,0.4,1.0])); // merchant
                instances.push(Instance::from_pos_scale_color(vec3(3.0,0.5,0.0), vec3(0.6,1.0,0.6), [0.2,0.6,1.0,1.0])); // guard
