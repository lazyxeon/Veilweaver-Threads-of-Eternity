...
fn main() -> anyhow::Result<()> {
    ...
    let mut camera = Camera {
        ...
        yaw: -3.14/2.0, pitch: -0.5, fovy: 60f32.to_radians(),
        aspect: 16.0/9.0, znear:0.1, zfar: 500.0
    };
    ...
    for t in &nav.tris {
        instances.push(Instance::from_pos_scale_color(t.center + vec3(0.0,0.05,0.0), vec3(0.1,0.1,0.1), [0.7,0.7,0.3,1.0]));
    }
    // visualize path
    for p in &path {
        instances.push(Instance::from_pos_scale_color(*p + vec3(0.0,0.08,0.0), vec3(0.12,0.12,0.12), [0.2,1.0,0.4,1.0]));
    }
...
#[inline]
fn tri(a: glam::Vec3, b: glam::Vec3, c: glam::Vec3) -> Triangle { Triangle{ a,b,c } }
