...
fn main() -> anyhow::Result<()> {
    ...
    let snap = WorldSnapshot {
        ...
        enemies: vec![ EnemyState{ id: boss, pos: w.pos_of(boss).unwrap(), hp:300, cover:"high".into(), last_seen:w.t } ],
        pois: vec![], objective: Some("defeat_boss".into()),
    };
    let mut budget = DirectorBudget { traps: 2, terrain_edits: 3, spawns: 2 };
    let mut pd = PhaseDirector::new(vec![
        PhaseSpec{ name:"Dreadwatch".into(), hp_threshold: 250, terrain_bias: 0.6, aggression: 0.3 },
        PhaseSpec{ name:"Lashing Gale".into(), hp_threshold: 150, terrain_bias: 0.3, aggression: 0.6 },
        PhaseSpec{ name:"Terminal Spiral".into(), hp_threshold: 50, terrain_bias: 0.7, aggression: 0.9 },
    ]);
