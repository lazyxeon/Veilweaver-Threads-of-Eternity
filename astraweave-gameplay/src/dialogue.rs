...
#[derive(Clone, Debug, Serialize, Deserialize)]
pub struct Line { pub speaker: String, pub text: String, #[serde(default)] pub set_vars: Vec<(String,String)> }

#[derive(Clone, Debug, Serialize, Deserialize)]
pub struct Choice { pub text: String, pub go_to: String, #[serde(default)] pub require: Vec<Cond> }
...
impl DialogueState {
    pub fn new(d: &Dialogue) -> Self {
        let map: HashMap<String, usize> = d.nodes.iter().enumerate().map(|(i,n)| (n.id.clone(), i)).collect();
        let idx = *(&map).get(&d.start).expect("start");
        Self { idx, map, vars: HashMap::new() }
    }
    ...
    pub fn choose(&mut self, d: &Dialogue, choice_idx: usize) -> bool {
        ...
        if let Some(c) = n.choices.get(choice_idx) {
            if !c.require.iter().all(|cond| eval(cond, &self.vars)) { return false; }
            if let Some(&ni) = self.map.get(&c.go_to) {
                // apply set_vars of next line when we move
                self.idx = ni;
                if let Some(l) = &d.nodes[ni].line {
                    for (k,v) in &l.set_vars { self.vars.insert(k.clone(), v.clone()); }
                }
                return true;
            }
        }
...
pub fn compile_banter_to_nodes(id: &str, src: &str) -> Dialogue {
    ...
    for (ln, line) in src.lines().enumerate() {
        ...
        if let Some(rest) = line.strip_prefix('[') {
            ...
            } else { ("Unknown".into(), line.into()) };
            let idn = format!("n{}", i);
            nodes.push(Node { id: idn.clone(), line: Some(Line{ speaker, text, set_vars: vec![] }), choices: vec![], end:false });
            last_id = idn; i += 1;
            ...
            if let Some(eq) = kv.find('=') {
                ...
                if let Some(n) = nodes.last_mut() {
                    if let Some(l) = n.line.as_mut() { l.set_vars.push((k,v)); }
                }
            }
        } else if let Some(rest) = line.strip_prefix('?') {
            // condition goto
            ...
            if parts.len() == 2 {
                let cond = parts[0];
                let goto = parts[1].strip_prefix("goto").map(|s| s.trim()).unwrap_or("n0").to_string();
                let mut conds = vec![];
                if cond.contains("==") {
                    let z: Vec<_> = cond.split("==").collect();
                    conds.push(Cond::Eq{ key:z[0].trim().into(), val:z[1].trim().into() });
                } else if cond.contains("!=") {
                    let z: Vec<_> = cond.split("!=").collect();
                    conds.push(Cond::Ne{ key:z[0].trim().into(), val:z[1].trim().into() });
                }
                if let Some(n) = nodes.last_mut() {
                    n.choices.push(Choice{ text:"Continue".into(), go_to: goto, require: conds });
                }
            }
        }
    }
    if let Some(n) = nodes.last_mut() { n.end = true; }
    Dialogue { id: id.into(), start: nodes.first().map(|n| n.id.clone()).unwrap_or("n0".into()), nodes }
}
