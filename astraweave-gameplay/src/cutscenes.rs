...
#[derive(Clone, Debug, Serialize, Deserialize)]
pub struct Timeline { pub cues: Vec<Cue> }
...
impl CutsceneState {
    pub fn new() -> Self { Self { idx: 0, t: 0.0 } }

    /// Advances timeline; returns camera override (pos, yaw, pitch) and optional text
    pub fn tick(&mut self, dt: f32, tl: &Timeline) -> (Option<(Vec3, f32, f32)>, Option<String>, bool) {
        if self.idx >= tl.cues.len() { return (None, None, true); }
        self.t += dt;
        match &tl.cues[self.idx] {
            Cue::CameraTo { pos, yaw, pitch, time } => {
                if self.t >= *time {
